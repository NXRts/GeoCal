...
var NgZoneChangeDetectionScheduler = class _NgZoneChangeDetectionScheduler {
  ...
  static {
    this.ɵfac = function NgZoneChangeDetectionScheduler_Factory(
      __ngFactoryType__,
    ) {
...
var ChangeDetectionSchedulerImpl = class _ChangeDetectionSchedulerImpl {
  ...
  static {
    this.ɵfac = function ChangeDetectionSchedulerImpl_Factory(
      __ngFactoryType__,
    ) {
...
function getPlatform() {
  const isServer =
    _platformInjector && _platformInjector.get(ALLOW_MULTIPLE_PLATFORMS, false);
  if (isServer) {
    return null;
  }
...
function internalCreateApplication(config) {
  ...
  try {
    ...
    if (
      platformInjector.get(ALLOW_MULTIPLE_PLATFORMS, false) === true &&
      !config.platformRef
    ) {
      throw new RuntimeError(
        401,
        ngDevMode &&
          "Missing Platform: This may be due to using `bootstrapApplication` on the server without passing a `BootstrapContext`. Please make sure that `bootstrapApplication` is called with a `BootstrapContext.",
      );
    }
    if (
      (typeof ngDevMode === "undefined" || ngDevMode) &&
