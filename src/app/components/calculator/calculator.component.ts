...
import { GeometryService, ShapeDefinition } from '../../services/geometry.service';
...
export class CalculatorComponent implements OnInit, OnDestroy {
  ...
  selectShape(shapeKey: string): void {
    ...
    shape.inputs.forEach(input => {
      ...
      const isOptional = shape.key === 'triangle' && (input.name === 'sideA' || input.name === 'sideB');
    ...
    this.formSubscription?.unsubscribe();
    this.formSubscription = this.calculatorForm.valueChanges.subscribe(values => {
      this.errorMessage = '';
      shape.inputs.forEach(input => {
        ...
        if (val !== null && val !== undefined && val !== '' && Number(val) >= 0) {
          ...
          this.svgValues[input.name] = input.name === 'radius' ? 'r' : 
                                      input.name === 'width' ? 'w' : 
                                      input.name === 'height' ? 'h' : 
                                      input.name === 'base' ? 'a' : 
                                      input.name === 'side' ? 's' : input.name;
        }
      });
    });
  }
  ...
  calculate(): void {
    if (this.calculatorForm.invalid) {
      ...
      this.errorMessage = 'Silakan isi semua parameter yang diperlukan dengan angka positif.';
      return;
    }
    ...
    const result = this.geometryService.calculate(this.selectedShape.key, numericValues);
