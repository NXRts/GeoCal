  ...
  private shapeDefinitions: Record<string, ShapeDefinition> = {
    circle: {
      ...
      formula(values) {
        ...
        if (r === undefined || isNaN(r) || r <= 0) return { error: 'Masukkan jari-jari yang valid (> 0).' };
    ...
    rectangle: {
      ...
      formula(values) {
        ...
        if (w === undefined || isNaN(w) || w <= 0 || h === undefined || isNaN(h) || h <= 0) {
    ...
    triangle: {
      ...
      inputs: [
        ...
        { name: 'sideA', label: 'Sisi A (opsional untuk keliling)', type: 'number', min: 0, step: 'any', placeholder: 'Masukkan sisi A' },
        { name: 'sideB', label: 'Sisi B (opsional untuk keliling)', type: 'number', min: 0, step: 'any', placeholder: 'Masukkan sisi B' },
      ],
      formula(values) {
        ...
        if (a === undefined || isNaN(a) || a <= 0 || t === undefined || isNaN(t) || t <= 0) {
        ...
        if (sideA > 0 && sideB > 0) {
          ...
          results.push('Keliling: Masukkan Sisi A dan Sisi B (> 0) untuk menghitung keliling.');
        }
        return { results };
      },
    },
    ...
    cylinder: {
      ...
      formula(values) {
        const r = Number(values['radius']);
        const h = Number(values['height']);
        if (r === undefined || isNaN(r) || r <= 0 || h === undefined || isNaN(h) || h <= 0) {
    ...
    cube: {
      ...
      formula(values) {
        ...
        if (s === undefined || isNaN(s) || s <= 0) return { error: 'Masukkan panjang sisi yang valid (> 0).' };
